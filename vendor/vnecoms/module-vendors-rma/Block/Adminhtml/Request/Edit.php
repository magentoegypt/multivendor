...
class Edit extends \Magento\Backend\Block\Widget\Form\Container
{
    ...
    protected function _construct()
    {
        ...
        if($status->getCode() == \Vnecoms\RMA\Model\Request::STATUS_PACKSENT
            ...
            $this->buttonList->add(
                ...
                [
                    ...
                    'onclick'   => 'setLocation(\'' . $this->getPackageReceivedUrl($request) . '\')',
                ],
                10
            );
        }
            ...
            if ($status->getCode() == \Vnecoms\RMA\Model\Request::STATUS_RETURNED) {
                $this->buttonList->add(
                    ...
                    [
                        ...
                        'label' => __('Mark as Resolve'),
                        'onclick' => 'setLocation(\'' . $this->getCloseUrl($request) . '\')',
                    ],
                    10
                );
            }
        ...
        if (
            $status->getCode() == \Vnecoms\VendorsRMA\Model\Request::STATE_BEING ||
            $status->getCode() == \Vnecoms\VendorsRMA\Model\Request::STATE_AWAITING
        ) {
            $this->buttonList->add(
                'package_closed',
                [
                    'class' => 'action-scalable action-closed',
                    'label' => __('Mark as Resolve'),
                    'onclick' => 'setLocation(\'' . $this->getMakeCloseUrl($request) . '\')',
                ],
                10
            );
        }
    ...
    public function getDeleteRequestUrl($request) {
        return $this->getUrl('*/*/delete',array('request_id'=>$request->getId()));
    }
    ...
    /**
     * get Url Make close Request
     * @return mixed
     */
    public function getMakeCloseUrl($request) {
        return $this->getUrl('*/*/resolve',array('request_id'=>$request->getId()));
    }
