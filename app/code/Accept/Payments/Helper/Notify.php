			...
			if($hash === $_REQUEST['hmac']){
				if($_SERVER['REQUEST_METHOD'] === "POST"){	
					...
					if($data['success'] == 'true') {
						...
						/* try {
							$order = \Magento\Framework\App\ObjectManager::getInstance()->get('Magento\Checkout\Model\Session')->getLastRealOrder();
							$order->canceled();
							$order->save();
						} catch (\Exception $error) {
						...
						$this->messageManager->addError( __('Payment Voided'));
					}
