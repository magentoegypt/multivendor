...
class Data extends AbstractHelper
{
    ...
    public function sendCreditBalanceChangeEmail(
        ...
        $types = $this->scopeConfig->getValue(self::XML_PATH_CREDIT_BALANCE_CHANGE_RESTRICTION);
        $restrictionTypes = $types ? trim($types) : $types;
        if ($restrictionTypes) {
            $restrictionTypes = explode(",", $restrictionTypes);
            if (in_array($transaction->getType(), $restrictionTypes)) {
                return;
            }
        }
