...
class CreditmemoSaveAfter implements ObserverInterface
{
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        /* 
         * Check if system sent a notification sms to this creditmemo already
         * The creditmemo object does not support for isObjectNew method so we have to do it like this
         */
        $additionalData = 'vendor_creditmemo|'.$creditmemo->getId();
        ...
        if(!($vendorOrderId = $creditmemo->getVendorOrderId())) return;
        ...
        if($this->helper->canSendNewCreditmemoMessage($vendor->getId())){
            $message = $this->helper->getNewCreditmemoMessage();
            $this->filter->setVariables(['creditmemo' => $creditmemo]);
