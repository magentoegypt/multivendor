...
class RefundSpentCredit extends \Vnecoms\Credit\Model\Processor\AbstractProcessor
{
    ...
    public function process($data = [])
    {
        ...
        $transData = [
            ...
            'description'        => __("Refund spent credit(s) from cancelled order #%1", $data['order']->getIncrementId()),
            'vendor_description' => json_encode([$data['order']->getIncrementId()]),
    ...
    public function getDescription(\Vnecoms\Credit\Model\Credit\Transaction $transaction)
    {
        ...
        if (sizeof($transactionData) < 2) {
            return parent::getDescription($transaction);
        }
        ...
        $orderLink = '<a href="'.$this->urlBuilder->getUrl('sales/order/view', ['order_id'=>$order->getId()]).'">'.$order->getIncrementId().'</a>';
