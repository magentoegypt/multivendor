...
class Data extends \Magento\Tax\Helper\Data
{
    ...
    protected function calculateTaxForVendorItems(EntityInterface $order, $salesItem)
    {
        ...
        $jsonSerializer = \Magento\Framework\App\ObjectManager::getInstance()->get('Magento\Framework\Serialize\Serializer\Serialize');
        foreach ($items as $item) {
            ...
            foreach ($itemTaxDetails as $itemTaxDetail) {
               ...
               if ($itemTaxDetail->getItemId() == $orderItemId) {
                   ...
                   $ratio = $itemRatio;
                   if ($item->getTaxRatio()) {
                       $taxRatio = $jsonSerializer->unserialize($item->getTaxRatio());
