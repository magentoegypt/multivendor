...
class CreditCustom extends \Magento\Eav\Model\Entity\Attribute\Backend\AbstractBackend
{
    ...
    public function validate($object)
    {
        ...
        if ($value && is_array($value)) {
            if (!isset($value['from'])) {
                throw new LocalizedException(__("'From' value of Store Credit must be set '%1'", $attrCode));
            }
            if (!isset($value['to'])) {
                throw new LocalizedException(__("'To' value of Store Credit must be set '%1'", $attrCode));
            }
            return true;
