 ...
 */
namespace Vnecoms\VendorsRMA\Model\Request\Escalate;
...
class Template extends AbstractTemplate implements \Magento\Framework\Mail\TemplateInterface
{
    ...
    {
        $this->_init('Vnecoms\VendorsRMA\Model\ResourceModel\Template');
    }
    ...
    protected function _parseVariablesString($variablesString)
    {
        ...
        if ($variablesString && is_string($variablesString)) {
            ...
            $variables = \Laminas\Json\Json::decode($variablesString);
        }
        return $variables;
    }
    ...
    /**
     * get All template of vendor
     * @return array
     */
    public function getOptionVendorTemplate(){
        $collection = $this->getCollection()->addFieldToFilter("type_send_mail",2);
        $templateVendor = [];
        foreach($collection as $st){
            $templateVendor[$st->getId()] = $st->getTemplateCode();
        }
    ...
    /**
     * get All template of customer
     * @return array
     */
    public function getOptionCustomerTemplate(){
        $collection = $this->getCollection()->addFieldToFilter("type_send_mail",1);
        $templateCustomer = [];
        foreach($collection as $st){
            $templateCustomer[$st->getId()] = $st->getTemplateCode();
        }
    ...
    public function getCustomerMessage(){
        ...
        $customMessage = [];
        foreach($collection as $st){
            $customMessage[$st->getId()] = $st->getUseCustomMessage();
        }
