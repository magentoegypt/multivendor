 ...
 *
 * Price indexer resource model for the custom "new_bundle" product type.
 *
 * This is a faithful copy of \Magento\Bundle\Model\ResourceModel\Indexer\Price
 * (Magento 2.4.8). The ONLY behavioural difference is the product-type filter:
 * the core bundle indexer hard-codes `e.type_id = 'bundle'`, which silently
 * excludes our `new_bundle` products from catalog_product_index_price. Because
 * the frontend product collection INNER JOINs the price index, those products
 * never appear in category listings / search. Here the three SQL filters use
 * self::PRODUCT_TYPE ('new_bundle') instead.
 *
 * The whole class is duplicated (not subclassed) because the core implementation
 * builds its SELECTs inside `private` methods, which are not polymorphic and
 * therefore cannot be overridden.
 *
 * NOTE: if Magento_Bundle is upgraded, re-sync this file against the new core
 * indexer and re-apply the PRODUCT_TYPE substitution.
 *
 * @SuppressWarnings(PHPMD.CouplingBetweenObjects)
 */
namespace MagentoEgypt\BundleExtend\Model\ResourceModel\Indexer;
...
/**
 * "new_bundle" products Price indexer resource model
 *
 * @SuppressWarnings(PHPMD.CouplingBetweenObjects)
 */
...
{
    /**
     * Product type this indexer is responsible for.
     *
     * Core bundle indexer uses \Magento\Bundle\...\BundlePrice::PRODUCT_TYPE ('bundle').
     */
    public const PRODUCT_TYPE = BundleExtendHelper::NEW_BUNDLE_TYPE_CODE;
