...
class Tax extends \Magento\Sales\Model\Order\Creditmemo\Total\Tax
{
    ...
    public function collect(\Magento\Sales\Model\Order\Creditmemo $creditmemo)
    {
        ...
        if ($vendorOrder->getId()) {
            ...
            if ($creditmemo->getInvoice() || $creditmemo->getVendorInvoice()) {
                if ($vendorInvoice = $creditmemo->getVendorInvoice()) {
                    if ($baseOrderShippingAmount && $creditmemo->getBaseShippingAmount() !== null) {
                        ...
                        $shippingTaxAmount = $vendorInvoice->getShippingTaxAmount() * $taxFactor;
                        $baseShippingTaxAmount = $vendorInvoice->getBaseShippingTaxAmount() * $taxFactor;
                        $totalDiscountTaxCompensation += $vendorInvoice->getShippingDiscountTaxCompensationAmount() * $taxFactor;
                        $baseTotalDiscountTaxCompensation +=
                            $vendorInvoice->getBaseShippingDiscountTaxCompensationAmnt() * $taxFactor;
                        $shippingDiscountTaxCompensationAmount =
                            $vendorInvoice->getShippingDiscountTaxCompensationAmount() * $taxFactor;
                        ...
                        if ($taxFactor < 1 && $vendorInvoice->getShippingTaxAmount() > 0) {
