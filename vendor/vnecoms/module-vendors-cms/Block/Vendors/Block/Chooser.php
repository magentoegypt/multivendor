...
class Chooser extends \Vnecoms\Vendors\Block\Vendors\Widget\Grid\Extended
{
        ...
        $js = '
            function (grid, event) {
               // console.log(grid);
                ...
                var blockIdentifier = trElement.down("td").next(1).innerHTML.replace(/^\s+|\s+$/g,"");
               // console.log(blockIdentifier);
                '. $block. '.insertBlock(blockIdentifier);'.
            '}
        ';
    ...
    protected function _prepareColumns()
    {
        $this->addColumn(
            'chooser_id',
            ['header' => __('Block ID'), 'align' => 'right', 'index' => 'block_id', 'width' => 50]
        );
    ...
    {
        return $this->getUrl('cms/block/grid', ['_current' => true]);
    }
}
