...
// ObjectManager is responsible for creating instances of the specified classes
...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
	...
	public function getModel(){
		return ObjectManager::getInstance()->create('MGS\Portfolio\Model\Category');
	}

	public function getPortfolios($cateid = NULL){
		$portfolios = ObjectManager::getInstance()->create('MGS\Portfolio\Model\Portfolio')
			->getCollection()
			->addStoreFilter($this->_storeManager->getStore()->getId())
			->addFieldToFilter('status', 1);

		if($cateid != null){
			$resourceModel = ObjectManager::getInstance()->create('MGS\Portfolio\Model\ResourceModel\Portfolio');
	...
	public function getPortfolioAddress($portfolio){
		...
		if($identifier!=''){
			return $this->_getUrl('portfolio/'.$identifier);
		}
		return $this->_getUrl('portfolio/index/view', ['id'=>$portfolio->getId()]);
	}
	...
	public function getCategories($portfolio){
		$collection = ObjectManager::getInstance()->create('MGS\Portfolio\Model\Stores')
			->getCollection()
			->addFieldToFilter('portfolio_id', $portfolio->getId());

		$resourceModel = ObjectManager::getInstance()->create('MGS\Portfolio\Model\ResourceModel\Stores');
	...
	public function getCategoriesLink($portfolio){
		...
		if($collection->getSize()>0){
			$i=0;
			foreach($collection as $item){
				$cate = ObjectManager::getInstance()->create('MGS\Portfolio\Model\Category')
				->getCollection()
				->addFieldToFilter('category_id', ['eq' => $item->getCategoryId()])->getFirstItem();
				$i++;
				if($cate->getIdentifier()!=''){
					$html .= '<a href="'.$this->_getUrl('portfolio/'.$cate->getIdentifier()).'">'.$item->getName().'</a>';
				}else{
					$html .= '<a href="'.$this->_getUrl('portfolio/category/view', ['id'=>$cate->getId()]).'">'.$item->getName().'</a>';
				}
			}
		}
		return $html;
	...
	public function getMenu(){
		...
		foreach ($menu as $cate) {
			if($cate->getIdentifier()!=''){
				$cate->setLinkCate($this->_getUrl('portfolio/'.$cate->getIdentifier()));
			}else{
				$cate->setLinkCate($this->_getUrl('portfolio/category/view', ['id'=>$cate->getId()]));
			}
	...
	public function truncate($content, $length){
		return $this->_filter->truncate($content, ['length' => $length, 'etc' => '']);
	}
