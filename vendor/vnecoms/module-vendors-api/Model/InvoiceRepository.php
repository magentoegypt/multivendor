...
class InvoiceRepository  implements InvoiceRepositoryInterface
{
     ...
     * @param \Vnecoms\VendorsSales\Model\Order\InvoiceFactory $invoiceFactory
     */
    ...
    public function getList(
        ...
        foreach ($collection as $invoiceModel) {
            ...
            $this->addAdditionalInfoToInvoiceResult($invoiceData, $invoiceModel);
    ...
    /**
     * @param \Vnecoms\VendorsApi\Api\Data\Sale\InvoiceInterface $invoiceResult
     * @param $vendorInvoice
     */
    protected function addAdditionalInfoToInvoiceResult(
        \Vnecoms\VendorsApi\Api\Data\Sale\InvoiceInterface $invoiceResult,
        $vendorInvoice
    ){
        $vendorInvoice = $this->invoiceFactory->create()->load($vendorInvoice->getEntityId());
        $order = $vendorInvoice->getInvoice()->getOrder();
        $invoiceResult->setVendorOrder($vendorInvoice->getVendorOrder());
        $invoiceResult->setInvoice($vendorInvoice->getInvoice());
        $invoiceResult->setBillingAddress($order->getBillingAddress());
        $invoiceResult->setShippingAddress($order->getShippingAddress());
        $items = $vendorInvoice->getAllItems();
        $itemsData = [];
        foreach ($items as $item) {
            $itemsData[] = $item->getData();
        }
        $invoiceResult->setItems($itemsData);
    }
