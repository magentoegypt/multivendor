...
require([
	...
	function panelAction(){
		...
		if($('.panel-input-color').length){
			...
			$('.panel-input-color').on('change', function(){
			...
			$('.panel-input-color').each(function(){
				...
				inputColor.on('colorpicked', function () {
					progress.show();
					var loadStyle = 0;
			...
			$('.checkbox-temp').on('click', function(){
				...
				$('#'+realElId).val(elementValue).trigger('change');
			});
			...
			$('.multiple-checkbox').on('click', function(){
				...
				inputValue.toString();
				inputEl.val(inputValue).trigger('change');
			});

			/* Slider range input */
			$('.slider-input .slider').on('change', function(){
				var sliderEl = $(this);
				var dataInput = sliderEl.attr('data-input');
				$('#'+dataInput).val(sliderEl.val()).trigger('change');
			});
