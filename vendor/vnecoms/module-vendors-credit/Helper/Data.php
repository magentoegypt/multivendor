...
class Data extends AbstractHelper
{
    ...
    const XML_PATH_ENABLE_CREDIT = 'vendors/credit/enable_credit';
    ...
    const XML_PATH_ALLOW_VENDOR_REFUND_CREDIT = 'vendors/sales/allow_refund_credit';
    ...
    public function sendNewWithdrawalNotificationToAdmin(
        ...
        if (!$adminEmails || !trim($adminEmails)) {
            return;
        }
    ...
    public function sendEscrowNotificationEmail(\Vnecoms\VendorsCredit\Model\Escrow $escrow)
    {
        ...
        $escrow->setData('created_at_formated', $this->_localeDate->formatDateTime(
            $escrow->getCreatedAt() ? $escrow->getCreatedAt() : (new \DateTime())->getTimestamp(),
     ...
     * @return boolean
     */
    public function isEnabledEscrowTransaction($vendorGroupId = null){
        $isEnable = $this->scopeConfig->getValue('vendors/credit/enable_escrow');

        $groupIgnore = $this->scopeConfig->getValue('vendors/credit/vendor_group_restriction');
        if ($groupIgnore) {
            $groupIgnore = explode("," , $groupIgnore);
        } else {
            $groupIgnore = [];
        }
        ...
        $isEnableGroup = !in_array($vendorGroupId, $groupIgnore) ? true : false;
        return $isEnableGroup && $isEnable;
    }
    ...
    protected function formatWithdrawalData(
        ...
        $date = $withdrawal->getCreatedAt() ?  $this->_localeDate->formatDateTime(
     ...
     * @return int
     */
    public function isAllowRefundCredit()
    {
        return $this->scopeConfig->getValue(self::XML_PATH_ALLOW_VENDOR_REFUND_CREDIT);
    }
    ...
    {
        return $this->scopeConfig->getValue(self::XML_PATH_ENABLE_CREDIT);
    }
