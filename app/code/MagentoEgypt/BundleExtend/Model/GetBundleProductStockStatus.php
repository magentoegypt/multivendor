...
namespace MagentoEgypt\BundleExtend\Model;
 ...
 */
class GetBundleProductStockStatus extends \Magento\InventoryBundleProduct\Model\GetBundleProductStockStatus
{
    /**
        ...
        foreach ($bundleSelections->getItems() as $selection) {
            if ((int)$selection->getStatus() === Status::STATUS_ENABLED) {
                if($selection->getTypeId() == ConfigurableType::TYPE_CODE) {
                    foreach ($selection->getTypeInstance()->getUsedProducts($selection) as $child) {
                        $skuRequests[] = $this->isProductSalableForRequestedQtyRequestFactory->create(
                            [
                                'sku' => (string)$child->getSku(),
                                'qty' => $this->getRequestedQty($child, $stockId),
                            ]
                        );
                    }
                } else {
