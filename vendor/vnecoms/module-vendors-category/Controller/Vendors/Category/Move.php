...
class Move extends \Vnecoms\VendorsCategory\Controller\Vendors\Category
{
    /**
    ...
    public function __construct(
        ...
        \Vnecoms\VendorsCategory\Helper\Data $helperConfig
    ) {
    ...
    public function execute()
    {
        ...
        try {
            if(!$this->checkCreateSubCategory($parentNodeId)){
                throw new \Exception(__('Cannot create sub category.'));
            }
     ...
     * @return bool
     */
    protected function checkCreateSubCategory($pId){
        $category = $this->_objectManager->create('Vnecoms\VendorsCategory\Model\Category')->load($pId);
        if($category->getLevel() != 0 && !$this->_helperConfig->getEnabledVendorSubCat()){
                return false;
        }
        return true;
    }
