...
class Dashboard extends \Magento\Backend\Block\Template
{
    ...
    {
        $total = $this->transactionFactory->create()->getResource()->getTotalSpentCredit();
        if (!$total) return 0;
        return abs($total);
    }
    ...
    public function getCustomerCreditBalanceData()
    {
        if (!$this->getData('customer_credit_balance_data')) {
            ...
            $creditCollection->join(['customer_flat'=>$creditCollection->getTable('customer_grid_flat')], "customer_flat.entity_id=customer_id", ['email'=>'email','name'=>'name'], null, 'left');
            ...
            if ($totalCredit > 0) {
                ...
                foreach ($creditCollection as $creditObj) {
                    ...
                    if (($totalPercentOfTop5 + $percent) > 100) {
