...
class MemoRepository implements MemoRepositoryInterface
{
    ...
    public function getList(
        ...
        foreach ($collection as $memoModel) {
            ...
            $this->addAdditionalInfoToMemoResult($memoData, $memoModel);
            $memos[] = $memoData;
        }
    ...
    /**
     * @param \Vnecoms\VendorsApi\Api\Data\Sale\MemoInterface $memoResult
     * @param $vendorMemo
     */
    protected function addAdditionalInfoToMemoResult(
        \Vnecoms\VendorsApi\Api\Data\Sale\MemoInterface $memoResult,
        $vendorMemo
    ){
        $vendorMemo = $this->creditmemo->load($vendorMemo->getEntityId());
        $order = $vendorMemo->getOrder();
        $memoResult->setBillingAddress($order->getBillingAddress());
        $memoResult->setShippingAddress($order->getShippingAddress());
        $items = $vendorMemo->getAllItems();
        $itemsData = [];
        foreach ($items as $item) {
            $itemsData[] = $item->getData();
        }
        $memoResult->setItems($itemsData);
    }
