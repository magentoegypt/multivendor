    ...
    public function getCreditmemo()
    {
        if ($this->_creditmemo === null) {
            ...
            if (!$this->_creditmemo->getId()) {
                ...
                $grandTotal = $this->_creditmemo->getGrandTotal()  + abs($this->_creditmemo->getCreditAmount()) - $oldShippingAmount - $oldShippingTaxAmount
                    + $vendorShippingAmount + $vendorShippingTaxmount
                ;
                $baseGrandTotal = $this->_creditmemo->getBaseGrandTotal() + abs($this->_creditmemo->getBaseCreditAmount())  - $oldBaseShippingAmount - $oldBaseShippingTaxAmount
                    + $baseVendorShippingAmount + $baseVendorShippingTaxAmount;

                if (abs($this->_creditmemo->getCreditAmount()) >= $grandTotal) {
                    $this->_creditmemo->setCreditAmount(-$grandTotal);
                    $this->_creditmemo->setBaseCreditAmount(-$baseGrandTotal);
                    $grandTotal = $baseGrandTotal = 0;
                }  else {
                    $grandTotal -= abs($this->_creditmemo->getCreditAmount());
                    $baseGrandTotal -= abs($this->_creditmemo->getBaseCreditAmount());
                }
