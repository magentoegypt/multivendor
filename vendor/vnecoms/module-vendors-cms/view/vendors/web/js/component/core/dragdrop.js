...
define(['jquery', 'jquery/ui', 'jquery/ui-touch-punch'], function (jQuery, jQueryUi) {
    ...
    DragDrop.prototype.initDrag = function (element, container) {
        container = container || jQuery(element).parents('.vendorscms-stage-container');
        jQuery(element).draggable({
            ...
            opacity: 0.7,
            helper: 'clone',
            connectToSortable: ".vendorscms-droppable",
            zIndex: 1000,
    ...
    DragDrop.prototype.onDragStop = function (event, ui) {
        Hook.trigger('vendorscms-stage-ui-updated', false, false, this.stage);
    };
    ...
    DragDrop.prototype.onDrag = function (event, ui) {
        Hook.trigger('vendorscms-stage-restore-empty-text', false, false, this.stage);
    };
    ...
    DragDrop.prototype.initSortable = function (element) {
        if (!jQuery(element).hasClass('.vendorscms-droppable')) {
            element = jQuery(element).find('>.vendorscms-droppable');
            if (!element) {
        ...
        jQuery(element).sortable({
            tolerance: 'pointer',
            connectWith: '.vendorscms-droppable',
            helper: 'clone',
            cancel: 'a',
            ...
            placeholder: {
                element: function (clone, ui) {
                    ...
                    if (clone.is('a')) {
                        ...
                        element.attr('style', '').addClass('vendorscms-entity vendorscms-entity-container vendorscms-preview').removeClass('vendorscms-draggable');
                        element.find('.vendorscms-content-type-icon').attr('style', '');
                        return element;
                    }

                    // Otherwise return a standard div
                    return jQuery('<div />').addClass('vendorscms-droparea')
                },
    ...
    DragDrop.prototype.onSort = function (event, ui) {
        Hook.trigger('vendorscms-stage-restore-empty-text', {rows: this._getEffectedRows(event, ui)}, false, this.stage);
        var placeholder = jQuery(ui.placeholder);
        if (jQuery(ui.item).is('a')) {
            placeholder.hide();
        }

        if (placeholder.prev().hasClass('vendorscms-column') && placeholder.next().hasClass('vendorscms-column')) {
            placeholder.hide();
        } else {
            placeholder.show();
        }
