 ...
 */
namespace Vnecoms\VendorsCredit\Block\Vendors\Order\Totals;
...
class Credit extends \Magento\Framework\View\Element\Template
{
    ...
    protected function _initCredit()
    {
        ...
        if ($this->_source instanceof \Vnecoms\VendorsSales\Model\Order) {
            $refundedCredit = $this->_source->getCreditRefunded();
            if (abs($refundedCredit) > 0) {
        ...
        if (!$usedCredit || abs($usedCredit) == 0) {
            return;
        }
        $totalData = new \Magento\Framework\DataObject([
            'code' => 'credit',
            'field' => 'credit',
            'strong' => false,
            'value' => -abs($usedCredit),
            'base_value' => -abs($baseUsedCredit),
