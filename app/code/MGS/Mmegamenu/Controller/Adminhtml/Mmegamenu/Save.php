...
class Save extends \MGS\Mmegamenu\Controller\Adminhtml\Mmegamenu
{
    ...
    public function execute()
    {
		...
		$repository = $this->_objectManager->create('\Magento\Store\Api\StoreRepositoryInterface');
            ...
            try {
				...
				foreach ($allStores as $_store) {
                    if($_store!=0){
                        ...
                        $_storeData['store_id'] = $_store;
                        $_storeData['parent_menu_id'] = $data['parent_id'];
                        $_storeData['html'] = $this->getMenuHtml($item, $_store);
    ...
    public function getCategoryUrl($category, $storeId)
    {
        ...
        if ($category instanceof Category) {
            ...
            if(($this->_storeManager->getStore()->getRootCategoryId() == $category->getId())){
                $url = $this->_storeManager->getStore($category->getStoreId($storeId))->getBaseUrl().$category->getUrlPath();
            }else{
	...
	public function getMenuHtml($item, $storeId) {
        if ($item['menu_type'] == 2) {
            return $this->getStaticMenu($item,$storeId);
        } else {
            return $this->getCategoryMenu($item, $storeId);
        }
    }

	public function getStaticMenu($item,$storeId) {

		if (filter_var($item['url'], FILTER_VALIDATE_URL)) {
			$itemUrl = $item['url'];
		}else{

			$itemUrl = $this->_storeManager->getStore($storeId)->getBaseUrl() .$item['url'];
		}
        ...
        if ($categoryId) {
            ...
            if ($item['url'] != '') {
                ...
                if ($this->_storeManager->getStore()->getRootCategoryId() == $category->getId()) {
					...
					$itemUrl = $this->getCategoryUrl($category,$storeId);
                }
            }
        }
	...
	public function drawList($category, $item, $level, $storeId) {
		...
		if ($category->getFbuilderThumbnail() !="" && $item['columns'] > 1 && $level == 1 && $item['use_thumbnail'] == 1) {
			...
			$html[] = '<a class="category-image" href="' . $this->getCategoryUrl($category,$storeId) . '"><img alt="'.$category->getName().'" class="img-fluid img-full" src="'.$imageCateUrl.'" /></a>';

        }

        $html[] = '<a href="' . $this->getCategoryUrl($category,$storeId) . '">';
