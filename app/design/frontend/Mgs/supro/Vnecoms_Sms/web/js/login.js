	...
	$.widget('vnecoms.smsLogin', {
		...
		_create: function() {
			...
			$.validator.addMethod(
				'validate-intlmask',
				function () {
					return self.options.validMask;
				},
				$.mage.__('The phone number entered is invalid.')
			);
		},
		...
		_bindEvents: function(){
            ...
            $(self.options.fieldCtrSelector+'.selected').click();
		},
		...
		_initMobileInput: function(){
			...
			$(this.options.mobileInput).on("paste", function(e) {
				var pastedText = '';
				...
				this.value = pastedText.replaceAll(" ", '');
				self._updateMobileNumber();
			});
		},
		_initMask: function(){
			...
			var self = this;
			var mask = intlTelInputUtils.getExampleNumber(countryData.iso2, true, numberType).replaceAll(' ','');
            $(this.options.mobileInput).attr('placeholder', mask);
			$(this.options.mobileInput)
				.mask(mask.replace(/([0-9])/g, '0'), {
					onKeyPress: function(cep){
						self.options.validMask = false;
					},
					onComplete: function(cep) {
						self.options.validMask = true;
						if(Array.from(mask)[0] == 0 && Array.from(cep)[0] != 0) {
							self.options.validMask = false;
						}
					}
				});
