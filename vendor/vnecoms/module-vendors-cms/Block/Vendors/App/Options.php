...
class Options extends \Vnecoms\Vendors\Block\Vendors\Widget\Form\Generic
{
    /**
    ...
    public function __construct(
        ...
        \Vnecoms\VendorsCms\Model\App $app,
        array $data = []
    ) {
    ...
    /**
     * Prepare App Options Form and values according to specified type.
     *
     * The type must be set in data before
    ...
    public function getMainFieldset()
    {
        ...
        $mainFieldsetHtmlId = 'options_fieldset'.md5($this->getType());
        $this->setMainFieldsetHtmlId($mainFieldsetHtmlId);
        $fieldset = $this->getForm()->addFieldset(
            $mainFieldsetHtmlId,
            ['legend' => __('Frontend Application Options'), 'class' => 'fieldset-wide fieldset-app-options box box-primary']
        );
    ...
    public function addFields()
    {
        ...
        if (!$this->getType()) {
            throw new \Magento\Framework\Exception\LocalizedException(__('Please specify a Application Type.'));
        }
        //get config of application
        $config = $this->_app->getConfigAsObject($this->getType());
