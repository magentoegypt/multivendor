...
class ActivateTheme implements ObserverInterface
{
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        if (!empty($keyValue)) {
            ...
            curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query(array('theme'=>'Supro', 'domain' => $domain, 'purchase_code' => $keyValue, 'magento_version' => $magentoVersion)));
            ...
            if($response['status']=='success'){
                ...
                $this->messageManager->addError($response['message'] ?? __('Supro theme has not been activated') );
            }
            curl_close($ch);
        }else{
