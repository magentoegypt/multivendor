...
use Vnecoms\RMA\Model\Request\Template\TransportBuilder;
...
class UploadTransportBuilder extends \Magento\Framework\Mail\Template\TransportBuilder
{
    ...
    const TYPE_RAR                 = 'application/rar';

    /**
     * @var \Magento\Framework\Mail\Template\TransportBuilder
     */
    protected $_mailTransportFactoryTicket;
    /**
     ...
     * @param array $parts
     */
    protected function initMessageParts($parts){
        return array_merge($parts, $this->messagePath);
    }
    ...
    protected function prepareMessage()
    {
        ...
        $this->messageData['body'] = $this->mimeMessageInterfaceFactory->create(
            ['parts' => $this->initMessageParts([$mimePart])]
        );
     ...
     * @return $this
     */
    public function addAttachmentFiles($files, $name = null)
    {
        $path = [];
        if (is_array($files) && count($files)) {
            foreach ($files as $namefile => $file) {
                if (!empty($file) && file_exists($file)) {
                    $type = $this->getFileType($namefile);
                    $path[] = $this->createAttachment(
    ...
    {
        $type = null;
        $path_attachment=pathinfo($fileName);
     ...
     * @return mixed
     */
    public function createAttachment($content, $fileName, $fileType = 'application/pdf') {
        $attachmentPart = $this->partFactory->create(['content' => $content]);
