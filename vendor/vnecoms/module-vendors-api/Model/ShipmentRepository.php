...
class ShipmentRepository implements ShipmentRepositoryInterface
{
    ...
    public function getList(
        ...
        foreach ($collection as $shipmentModel) {
            ...
            $this->addAdditionalInfoToShipmentResult($shipmentData, $shipmentModel);
    ...
    /**
     * @param \Vnecoms\VendorsApi\Api\Data\Sale\ShipmentInterface $shipmentResult
     * @param $shipmentModel
     */
    protected function addAdditionalInfoToShipmentResult(
        \Vnecoms\VendorsApi\Api\Data\Sale\ShipmentInterface $shipmentResult,
        $shipmentModel
    ){
        $shipmentModel = $this->shipmentFactory->load($shipmentModel->getEntityId());
        $order = $shipmentModel->getOrder();
        $shipmentResult->setBillingAddress($order->getBillingAddress());
        $shipmentResult->setShippingAddress($order->getShippingAddress());
        $items = $shipmentModel->getAllItems();
        $itemsData = [];
        foreach ($items as $item) {
            $itemsData[] = $item->getData();
        }
        $shipmentResult->setItems($itemsData);
    }
