...
class InvoiceService extends \Magento\Sales\Model\Service\InvoiceService
{
    ...
    protected function reloadDiscount($invoice, $vendorOrder) {
        ...
        $baseGrandTotal = $invoice->getBaseGrandTotal()
            + abs($invoice->getBaseCreditAmount())
        ...
        if (abs($invoice->getCreditAmount()) >= $grandTotal) {
            $invoice->setCreditAmount(-$grandTotal);
            $invoice->setBaseCreditAmount(-$baseGrandTotal);
            $grandTotal = $baseGrandTotal = 0;
        } else {
            $grandTotal -= abs($invoice->getCreditAmount());
            $baseGrandTotal -= abs($invoice->getBaseCreditAmount());
        }
