...
class Validate extends \Vnecoms\VendorsProduct\Controller\Vendors\Product
{
    ...
    private function _processAfterSpecialAttribute($product) {
        $ignoreAttribute = $this->_objectManager->get(\Vnecoms\VendorsProduct\Helper\Data::class)->getNotUsedVendorAttributes();
        ...
        if (!$product->getSku() && in_array("sku", $ignoreAttribute)) {
            $product->setSku($urlKey);
        }

        if (!$product->getUrlKey() && in_array("url_key", $ignoreAttribute) ) {
