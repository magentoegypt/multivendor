...
class WireTransfer extends AbstractMethod
{
    ...
    public function isEnteredMethodInfo($vendorId)
    {
        ...
        $bankName = $bankName ? trim($bankName) : false;
        $swiftCode = $this->getSwiftCode($vendorId);
        $swiftCode = $swiftCode ? trim($swiftCode) : false;
        $accountHolder = $this->getAccountHolder($vendorId);
        $accountHolder = $accountHolder ? trim($accountHolder) : false;
        $accountNumber = $this->getAccountNumber($vendorId);
        $accountNumber = $accountNumber ? trim($accountNumber) : false;

        return $bankName && $swiftCode && $accountHolder && $accountNumber;
    }
