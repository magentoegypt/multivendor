...
//     }
    ...
    {
        if (!$this->_scopeConfig->getValue(\Vnecoms\Vendors\App\Area\FrontNameResolver::XML_PATH_CUSTOM_VENDOR_URL, 'store', $this->_getScope()->getId())) {
            return parent::getBaseUrl($params);
        }
        $customUrl = $this->_scopeConfig->getValue(\Vnecoms\Vendors\App\Area\FrontNameResolver::XML_PATH_CUSTOM_VENDOR_URL, 'store', $this->_getScope()->getId());
        $baseUrlInfo = parse_url($customUrl);
        $baseDomain = $baseUrlInfo["scheme"] ."://".$baseUrlInfo['host'];
        $oldBaseUrl = parent::getBaseUrl($params);

        $oldBaseUrlInfo = parse_url($oldBaseUrl);
        $oldBaseDomain = $oldBaseUrlInfo["scheme"] ."://".$oldBaseUrlInfo['host'];
